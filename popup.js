...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Setup UI components and load checkbox checked states
  ...
  // Fetch system fonts list and populate datalist
  ...
  // UX Improvement: Select all text on focus to show datalist dropdown instantly
  textFontInput.addEventListener('focus', function() {
    this.select();
  });
  textFontInput.addEventListener('blur', function() {
    if (!this.value.trim()) {
      this.value = textFont; // Restore saved font if left empty
    }
  });
  ...
  codeFontInput.addEventListener('blur', function() {
    if (!this.value.trim()) {
      this.value = codeFont; // Restore saved font if left empty
    }
  });
  ...
  // Handle Event Listeners for Switches
  ...
  // Handle Force Apply (Direct Injection with Clean-up)
  forceApplyBtn.addEventListener('click', async () => {
    ...
    chrome.scripting.executeScript({
      ...
      func: (isExtEnabled, selList, keepCodeLtr, applyRtlPrompts, applyRtlInput, fEnabled, tFont, tFontSize, cEnabled, cFont, cFontSize) => {
        // Clean up any dynamic "auto" sheet to prevent style clashes
        ...
        // Handle Google Font loading using <link> to force repaint on existing elements
        ...
        // If master switch is off, clear styles entirely
        if (!isExtEnabled) {
          styleEl.textContent = '';
          return;
        }
